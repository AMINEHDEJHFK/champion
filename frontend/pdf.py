...
from utils import path, HOME, AWAY
...
class PDF(FPDF):
    ...
    def set_team_target_header(self, row_index: int, team_name: str) -> int:
    ...
    def set_team_opponents(self, row_index: int, matches: dict) -> None:
    ...
    def generate(self):
        ...
        for club, matches in self.tirages.items():
            j = self.set_team_target_header(row_index=j, team_name=club)
            self.set_team_opponents(row_index=j, matches=matches)
            j += 1
